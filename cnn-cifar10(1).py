...
def do_train(train_dir, _labels, _features):
    ...
    with tf.Graph().as_default():
        ...
        # inference model.
        logits = build_model(_features)

        # Calculate loss.
        total_loss = loss(logits, _labels)
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            mon_sess.run(init)
            print("do_train graph2:", tf.get_default_graph())
...
def main():
    ...
    _labels, _features = read_data(SRC_TAR_DIR, DST_PATH, FILENAME, batch_size=BATCH_SIZE, num_epochs=2)

    do_train(train_dir, _labels, _features)
